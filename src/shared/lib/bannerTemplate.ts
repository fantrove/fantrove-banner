...
// Purpose: Single source of truth for banner rendering. v5: 2 modes.
//   builder — builds from JSON config, wraps in .be-banner
//   html    — mounts customHtml as-is inside Shadow DOM
...
export function applyTranslations(
  ...
  return html.replace(
    ...
    (match, _tag, key, inner) => match.replace(inner, esc(langMap[key] ?? key))
  );
}

// ── BASE CSS — used by builder mode inside Shadow DOM ─────────────────────────
// Inlined here so the SDK can inject it into the Shadow DOM <style> tag.
...
// ── buildBannerInnerHtml — builder mode ───────────────────────────────────────
...
// ── convertBuilderToHtml ──────────────────────────────────────────────────────
// Called when user switches builder → html mode.
// Generates complete HTML + CSS for each supported lang from builder state,
// so user can immediately see and edit the generated markup.
export function convertBuilderToHtml(
  ...
  const baseCss = String(data.bannerStyles ?? '');

  for (const lang of langs) {
    const inner = buildBannerInnerHtml(data, { lang });
    html[lang] = `<div class="be-banner">\n${inner}\n</div>`;
    css[lang]  = baseCss;
  }
...
// Generates a full srcdoc string for the preview iframe.
// Both modes render inside an isolated <style> block to simulate Shadow DOM.
export function buildPreviewDocument(data: BannerPublicPayload, lang = 'en'): string {

  if (data.editorMode === 'html') {
    // ── HTML mode: user's HTML + CSS, no wrapper ──────────────────────────
    ...
    const html    = applyTranslations(sanitizeHtml(rawHtml), data.translations ?? {}, lang);
...
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
body{padding:12px;background:#f0f2f5;font-family:Inter,system-ui,sans-serif;}
${rawCss}
</style>
</head>
<body>${html}</body>
  ...
  // ── Builder mode: .be-banner wrapper + BASE_CSS ───────────────────────────
